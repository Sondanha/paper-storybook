...
조건:
1. 장면 수는 반드시 10~12개여야 함. 10개 미만 또는 12개 초과는 규칙 위반으로 간주함.
2. 1개 또는 2개 장면만 출력하는 것은 절대 허용되지 않음.
3. 내레이션은 한국어로 작성하되, 주요 용어는 영어 원어를 병기할 수 있음.
4. raw_text에는 해당 장면에 대응되는 원문 일부를 반드시 포함시킬 것.
5. 저작권 문구, 참고문헌, 저자 정보, 이메일 주소, 출판사/저널 문구 등
   스토리라인과 무관한 메타데이터는 Scene에 포함하지 말 것.
6. 반드시 JSON 배열만 출력하고, 다른 문장/코멘트/코드펜스는 절대 포함하지 말 것.
"""
